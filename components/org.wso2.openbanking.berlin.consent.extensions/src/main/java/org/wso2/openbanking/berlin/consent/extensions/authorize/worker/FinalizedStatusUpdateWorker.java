...
/**
 * Class to update authorization object to finalized status.
 */
public class FinalizedStatusUpdateWorker extends BerlinAuthStatusUpdateWorker {
    @Override
    boolean isCurrentAuthorizationStatusEligible(String authStatus) {
        return StringUtils.equals(ScaStatusEnum.PSU_AUTHENTICATED.toString(), authStatus) ||
                StringUtils.equals(ScaStatusEnum.STARTED.toString(), authStatus);
    }

    @Override
    String getNewAuthStatusValue() {
        return ScaStatusEnum.FINALISED.toString();
    }
}
